...
{
  console.log("[145] ambient FF: contact without SHIFT; declare stays SHIFT-only");
  const { AgentPlayer } = await import("../server/agent");
  const { mock } = await import("../server/llm");
...
// ------------------------------------------------- 146. plans.jsonl FF senses join
// partnerStrike + cumulative partner→me dmg at plan time (author Artem 2026-08-24)
{
  console.log("[146] plans.jsonl: partnerStrike + partnerAccidentalDmg at plan time");
  const { AgentPlayer } = await import("../server/agent");
  const { mock } = await import("../server/llm");
